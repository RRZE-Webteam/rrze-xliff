...
(function(){
    ...
    xliffEmailExportButton.addEventListener("click", function(e){
        let xliffEmailExportField = document.querySelector("#xliff_export_email_address"),
			...
			xliffTargetLangCodeSelect = document.querySelector("#xliff_export_target_lang_code"),
			xliffTargetLangCode = '';
        e.preventDefault();
		...
		if (xliffTargetLangCodeSelect) {
			xliffTargetLangCode = xliffTargetLangCodeSelect.value;
		}
        ...
        xhr.send(`_nonce=${rrzeXliffJavaScriptData.nonce}&action=xliff_email_export&xliff_export_post=${rrzeXliffJavaScriptData.post_id}&xliff_export_email_address=${xliffEmailExportField.value}&email_export_note=${xliffEmailNote}&xliff_target_lang_code=${xliffTargetLangCode}`);
	});
	
	// Die Zielsprache bei XLIFF-1-Export beim Runterladen eines Exports an den Link übergeben.
	const xliffDownloadLink = document.querySelector( '.rrze-xliff-export-modal-wrapper .xliff-export-download-link');
	if (xliffDownloadLink) {
		xliffDownloadLink.addEventListener('click', function(e) {
			let xliffTargetLangCodeSelect = document.querySelector("#xliff_export_target_lang_code"),
			xliffTargetLangCode = '';
			...
			xliffDownloadLink.setAttribute('href', `${xliffDownloadLink.getAttribute('href')}&xliff_target_lang_code=${xliffTargetLangCode}`);
		});
	}
